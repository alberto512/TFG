...
const StatsPage = () => {
  ...
  const [balancesPositive2, setBalancesPositive2] = useState<Balances2>({
    nodes: [],
    links: [],
  })
  const [balancesNegative2, setBalancesNegative2] = useState<Balances2>({
    nodes: [],
    links: [],
  })
    ...
    axios
      ...
      .then((response) => {
        let balancesPositive: Balances2 = {
          nodes: [
            {
              name: accounts.find((account) => account.id === accountSelected)!
                .name,
            },
          ],
          links: [],
        ...
        for (const i in response.data) {
          if (response.data[i].amount > 0) {
            ...
            if (
              nodes.some((node) => node.name === response.data[i].category.name)
            ) {
              links.push({
                source: 0,
                target: nodes.indexOf(
                  nodes.find(
                    (node) => node.name === response.data[i].category.name
                  )!
                ),
      ...
      .then((response) => {
        ...
        for (let i in response.data) {
          for (let j in response.data[i].transactions) {
            if (
              response.data[i].transactions[j].category.id ===
        ...
        let data = dateValues.map((value) => {
          ...
          dataObj[
            categories.find(
              (category) => category.id === categoriesPositionSelected[0]
            )!.name
          ] = 0
          dataObj[
            categories.find(
              (category) => category.id === categoriesPositionSelected[1]
            )!.name
          ] = 0
          return dataObj
        })
      ...
      <div className={styles.body}>
        <div className={styles.typeStats}>
          <div className={styles.selectWrapper}>
            <InputSelect
              placeholder='Select a type of stat'
              options={statTypes}
        ...
        <div className={styles.contentWrapper}>
          ...
          {statType === '1' && statsReady && (
            <CategoriesBalances
              balancesPositive={balancesPositive1}
              balancesNegative={balancesNegative1}
            />
          )}
          {statType === '2' && statsReady && (
            <AccountBalances
              balancesPositive={balancesPositive2}
              balancesNegative={balancesNegative2}
            />
          )}
          {statType === '3' && statsReady && (
